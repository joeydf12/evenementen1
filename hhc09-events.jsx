...
const DEFAULT_ADMIN_PINS = [{ pin:"1909", role:"super" }];
...
function downloadAllICS(events) {
  const fmt = d => new Date(d).toISOString().replace(/[-:]/g,"").slice(0,15) + "Z";
  const lines = ["BEGIN:VCALENDAR","VERSION:2.0","PRODID:-//HHC09//Events//NL"];
  events.forEach(ev => lines.push("BEGIN:VEVENT",`UID:${ev.id}@hhc09.nl`,`DTSTART:${fmt(ev.start_time)}`,`DTEND:${fmt(ev.end_time||ev.start_time)}`,`SUMMARY:${ev.title}`,`LOCATION:${ev.location||""}`,`DESCRIPTION:${(ev.description||"").replace(/\n/g,"\\n")}`,"END:VEVENT"));
  lines.push("END:VCALENDAR");
  const a = document.createElement("a");
  a.href = URL.createObjectURL(new Blob([lines.join("\r\n")], { type:"text/calendar" }));
  a.download = "hhc09-agenda.ics"; a.click(); URL.revokeObjectURL(a.href);
}
...
function checkAndNotify(events) {
  ...
  const now = new Date();
  events.forEach(ev => {
    if (!isUpcoming(ev.start_time)) return;
    const hours = (new Date(ev.start_time) - now) / 3600000;
    if (hours >= 23.5 && hours <= 24.5) {
      const key = `notified_${ev.id}`;
      if (!localStorage.getItem(key)) {
        new Notification(`Morgen: ${ev.title}`, { body:`${formatDate(ev.start_time)} om ${formatTime(ev.start_time)}\n📍 ${ev.location||""}`, icon:"/icon-192.png" });
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" style={{ maxWidth:440 }} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{ display:"flex", gap:8 }}>
          <input className="input" value={name} onChange={e=>setName(e.target.value)} onKeyDown={e=>e.key==="Enter"&&name&&(onRegister(event.id,name),setName(""))} placeholder="Jouw naam" style={{ flex:1 }} />
          <button className="btn-red" onClick={()=>{if(name){onRegister(event.id,name);setName("");}}} disabled={!name}>Ik kom!</button>
        </div>
...
// ---- ADMIN DASHBOARD ----
function AdminDashboard({ events, attendees, primaryColor, allCategories, categoryColors, onSelectEvent }) {
  const now = new Date();
  ...
  return (
    <div>
      <div style={{ fontSize:13, fontWeight:700, textTransform:"uppercase", letterSpacing:2, color:primaryColor, marginBottom:20 }}>Dashboard</div>
...
export default function HHCEvents() {
  ...
  const [adminPins, setAdminPins] = useState(() => {
    const loaded = loadLS("hhc09_admin_pins", DEFAULT_ADMIN_PINS);
    if (loaded.length > 0 && typeof loaded[0] === "string") return loaded.map(pin => ({ pin, role:"super" }));
    return loaded;
  });
  ...
  useEffect(() => { load(); requestNotifications(); }, []);
  useEffect(() => { if (events.length) checkAndNotify(events); }, [events]);
  ...
  useEffect(() => { saveLS("hhc09_admin_pins", adminPins); }, [adminPins]);
  ...
  async function load() {
    ...
    try {
      ...
      setAttendees(attMap);
      saveLS("hhc09_events_cache", { events:evList, attendees:attMap, ts:Date.now() });
      setOffline(false);
    } catch {
      setOffline(true);
      if (!cached) setEvents([]);
  ...
  async function handleSave() {
    ...
    if (editingEvent) await sb(`events?id=eq.${editingEvent.id}`, "PATCH", payload);
    else await sb("events", "POST", payload);
  ...
  async function handleArchive(ev) {
    await sb(`events?id=eq.${ev.id}`, "PATCH", { archived: !ev.archived });
    await load();
  ...
  function submitPin() {
    const found = adminPins.find(p => p.pin === pinInput);
    if (found) {
      setAdminMode(true); setAdminRole(found.role); setShowPinModal(false); setPinInput("");
      showToast(`Ingelogd als ${ROLE_LABELS[found.role]||found.role}`);
    } else {
      setPinError(true);
    }
  }
  ...
  const embedCode = `<iframe src="${window.location.origin}${window.location.pathname}?embed=1" width="100%" height="600" frameborder="0" style="border-radius:8px;border:1px solid #e7e0d0;"></iframe>`;
    ...
    <div style={{ minHeight:"100vh", background:"#f5f1e6", color:"#1c1c26", fontFamily:"'Barlow Condensed','Arial Narrow',Arial,sans-serif" }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .input{background:#ffffff;border:1px solid #d8cfb8;color:#1c1c26;padding:10px 14px;border-radius:6px;font-family:inherit;font-size:15px;width:100%;transition:border-color .2s}
        ...
        .tab-btn{background:transparent;border:none;color:#555;padding:10px 16px;cursor:pointer;font-family:inherit;font-size:16px;font-weight:700;letter-spacing:.5px;text-transform:uppercase;border-bottom:2px solid transparent;transition:all .2s;white-space:nowrap}
        .tab-btn.active{color:#1c1c26;border-bottom-color:${primaryColor}}
      ...
      <header className="no-print" style={{ background:"linear-gradient(135deg,#f5821f,#e8731a)", position:"relative", overflow:"hidden" }}>
        ...
        <div style={{ maxWidth:960, margin:"0 auto", padding:"20px 20px 18px", position:"relative", display:"flex", alignItems:"flex-end", justifyContent:"space-between", flexWrap:"wrap", gap:12 }}>
          <div>
            <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:6 }}>
              {clubSettings.logo && <img src={clubSettings.logo} alt="logo" style={{ height:34, borderRadius:"50%", background:"#fff", padding:2 }} onError={e=>e.target.style.display="none"} />}
          ...
          <div style={{ display:"flex", gap:8, alignItems:"center", paddingBottom:4, flexWrap:"wrap" }}>
            {adminMode && <span style={{ fontSize:11, color:"#1d2d6b", background:"#ffffffcc", padding:"3px 10px", borderRadius:12, fontWeight:700, textTransform:"uppercase" }}>{ROLE_LABELS[adminRole]||"Admin"}</span>}
            {notifGranted
              ? <span style={{ fontSize:11, color:"#fff", fontFamily:"Barlow,sans-serif" }}>🔔 Notificaties aan</span>
              : <button className="btn-ghost-onbrand" onClick={()=>{ requestNotifications(); setTimeout(()=>setNotifGranted(Notification.permission==="granted"),500); }} style={{ fontSize:12 }}>🔔 Notificaties</button>
            }
            <button className="btn-ghost-onbrand" onClick={()=>downloadAllICS(events.filter(e=>!e.hidden&&!e.archived))} style={{ fontSize:12 }}>📅 Export .ics</button>
            {adminMode ? (
              <>
                ...
                <button className="btn-ghost-onbrand" onClick={()=>{ setAdminMode(false); setAdminRole(null); }} style={{ fontSize:12 }}>Uitloggen</button>
              </>
            ) : (
      ...
      {/* Tabs */}
      <div className="no-print" style={{ background:"#ffffff", borderBottom:"1px solid #e7e0d0" }}>
        <div style={{ maxWidth:960, margin:"0 auto", padding:"0 20px", display:"flex", gap:0, overflowX:"auto" }}>
          {[
            ...
            ["archief","Archief"],
            ["statistieken","Stats"],
            ...(adminMode ? [["dashboard","Dashboard"]] : []),
          ].map(([id,label])=>(
            <button key={id} className={`tab-btn ${tab===id?"active":""}`} onClick={()=>setTab(id)}>{label}</button>
          ))}
          <button className="tab-btn" onClick={()=>setShowEmbed(true)} style={{ marginLeft:"auto", fontSize:13 }}>{"</>"} Embed</button>
        </div>
      </div>
      ...
      <div className="print-title" style={{ padding:"20px 20px 0", fontSize:24, fontWeight:700 }}>HHC'09 Agenda — {MONTHS_NL[calDate.month]} {calDate.year}</div>
      ...
      <main style={{ maxWidth:960, margin:"0 auto", padding:"24px 20px 80px" }}>
        ...
        {tab==="dashboard" && adminMode && (
          <AdminDashboard events={events} attendees={attendees} primaryColor={primaryColor} allCategories={allCategories} categoryColors={categoryColors} onSelectEvent={ev=>{ setSelectedEvent(ev); setTab("agenda"); }} />
        )}
          ...
          <div className="modal" style={{ maxWidth:620 }} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:10, marginBottom:24 }}>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
                ...
                <div><label style={{ fontSize:11, color:"#555", display:"block", marginBottom:5, fontWeight:700, letterSpacing:1, textTransform:"uppercase" }}>Logo URL</label><input className="input" value={clubSettings.logo} onChange={e=>setClubSettings(s=>({...s,logo:e.target.value}))} placeholder="https://..." /></div>
              </div>
            </div>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:6, marginBottom:12 }}>
              {adminPins.map((p,i) => (
                <div key={i} className="settings-row">
                  <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                    <span style={{ fontFamily:"Barlow,sans-serif", fontSize:14, letterSpacing:4 }}>{'•'.repeat(p.pin.length)}</span>
                  ...
                  {adminPins.length>1 && <button className="btn-sm" onClick={()=>setAdminPins(ps=>ps.filter((_,j)=>j!==i))} style={{ color:"#e63946" }}>Verwijderen</button>}
                </div>
              ))}
              <div style={{ display:"flex", gap:8, marginTop:6 }}>
                <input className="input" type="password" value={newPin} onChange={e=>setNewPin(e.target.value)} placeholder="Nieuwe pincode" style={{ flex:1 }} />
                <select className="input" value={newPinRole} onChange={e=>setNewPinRole(e.target.value)} style={{ width:"auto" }}>
                  {Object.entries(ROLE_LABELS).map(([k,v])=><option key={k} value={k}>{v}</option>)}
                </select>
                <button className="btn-sm" onClick={()=>{ if(newPin.length>=4){ setAdminPins(ps=>[...ps,{pin:newPin,role:newPinRole}]); setNewPin(""); } }}>Toevoegen</button>
              </div>
            </div>
      ...
      {showEmbed && (
        <div className="modal-overlay" onClick={()=>setShowEmbed(false)}>
          <div className="modal" style={{ maxWidth:560 }} onClick={e=>e.stopPropagation()}>
            <h2 style={{ fontSize:20, fontWeight:900, textTransform:"uppercase", marginBottom:8 }}>{"</>"} Embed code</h2>
            <p style={{ color:"#555", fontSize:14, fontFamily:"Barlow,sans-serif", marginBottom:16 }}>Kopieer en plak op je clubwebsite</p>
            <textarea readOnly value={embedCode} style={{ background:"#f5f1e6", border:"1px solid #d8cfb8", color:"#52b788", padding:"14px", borderRadius:6, fontFamily:"monospace", fontSize:12, width:"100%", height:100, resize:"none" }} onClick={e=>e.target.select()} />
            <div style={{ display:"flex", gap:10, marginTop:12 }}>
              <button className="btn-red" style={{ flex:1 }} onClick={()=>{ navigator.clipboard?.writeText(embedCode); showToast("Embed code gekopieerd"); }}>Kopiëren</button>
              <button className="btn-ghost" onClick={()=>setShowEmbed(false)}>Sluiten</button>
            </div>
          </div>
        </div>
      )}
